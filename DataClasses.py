...
# JMagat, 2021-Sep-05, modified to add code to complete assignment_09;
#------------------------------------------#
...
class Track():
    ...
    def position(self, value):
        if type(value) == int:
            if value < 1:
                raise Exception('Position cannot be zero or less than one.')
                self.__position = value
        else:
            raise Exception('Value not valid. Position must be an integer.')
    ...
    def title(self, value):
        if type(value) == str:
            ...
            raise Exception('Value not valid. Title must be string')
